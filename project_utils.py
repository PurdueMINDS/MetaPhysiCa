...
def get_all_metrics(test_pred_y, test_y, input_length=0):
    test_mse_per_task = ((test_y[:, input_length:] - test_pred_y[:, input_length:]) ** 2).mean(dim=[1,2])
    test_mse = test_mse_per_task.mean().item()
    test_rmse = np.sqrt(test_mse)

    test_nrmse = test_rmse / test_y[:, input_length:].std().item()

    test_results = {
        "mse_per_task": test_mse_per_task,
        "mse": test_mse,
        "rmse": test_rmse,
        "nrmse": test_nrmse,
        "example_test_y": list(test_y[:5].detach().cpu()),
        "example_test_pred_y": list(test_pred_y[:5].detach().cpu()),
    }
    return test_results
...
class LinearStepScheduler():
    ...
    def __init__(self, initial, final, epochs, step_size):

        self.constant_ends = epochs // 3
        self.epochs = epochs
        linear_epochs = epochs - 2 * self.constant_ends
        self.step_size = step_size 
        self.param = initial
        self.delta = (final - initial) * (step_size + 1) / linear_epochs
        self.epoch = 0

    def step(self):
        self.epoch += 1
        if self.epoch <= self.constant_ends:
            return
        if self.epochs - self.epoch < self.constant_ends:
            return
            
        if (self.epoch - self.constant_ends) % self.step_size == 0:
            self.param += self.delta
